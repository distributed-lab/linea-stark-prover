...
fn main() {
    ...
    let mut ranges_traces: Vec<Vec<RawRangeTrace>> = vec![vec![]; 32];
    ...
    for i in 0..1 {
        let trace = RawRangeTrace::read_file(&format!("../range_{}.bin", i));
        ranges_traces[trace.get_max_height().ilog2() as usize].push(trace);
    }

    for i in 0..32 {
        let permutation_trace = permutation_traces.pop().unwrap();
        let lookup_trace = lookup_traces.pop().unwrap();
        let range_trace = ranges_traces.pop().unwrap();

        if !permutation_trace.is_empty() || !lookup_trace.is_empty() || !range_trace.is_empty() {
            println!("Proving for height 2^{}: {}x lookups, {}x perms {}x ranges", 31 - i, lookup_trace.len(), permutation_trace.len(), range_trace.len());
            prove_linea(
