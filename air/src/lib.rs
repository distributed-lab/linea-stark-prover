        ...
        self.configs.iter().for_each(|c| {
            match c {
                AirConfig::Lookup(l) => {
                    ...
                    // for (columns_ids_ind, columns_ids) in l.b_columns_ids.iter().enumerate() {
                    //     let mut b_local_comb = AB::Expr::from(AB::F::ZERO);
                    //     for i_row in columns_ids {
                    //         b_local_comb = b_local_comb * alpha.clone() + local[*i_row];
                    //     }
                    //
                    //     let b_local_challenge = b_local_comb + delta.clone();
                    //     builder.assert_eq(
                    //         b_local_challenge * local[l.b_inverses_id[columns_ids_ind]],
                    //         AB::F::ONE,
                    //     );
                    // }
