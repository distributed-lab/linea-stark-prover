...
impl<F: Field> BaseAir<F> for LineaPermutationAIR {
    fn width(&self) -> usize {
        println!("self.width * 2 + 2 {}", self.width * 2 + 2);
...
//         for i in 0..self.width {
//             next_a_total *= next[i] + challenge.clone();
//         }
//
//         let mut next_b_total = AB::Expr::from(AB::F::ONE);
//         for i in self.width..self.width * 2 {
//             next_b_total *= next[i] + challenge.clone();
//         }
//
